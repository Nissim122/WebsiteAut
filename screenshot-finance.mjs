import puppeteer from 'puppeteer';
import path from 'path';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

const browser = await puppeteer.launch({
  headless: true,
  executablePath: 'C:/Users/nisim/.cache/puppeteer/chrome/win64-146.0.7680.153/chrome-win64/chrome.exe',
  args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-gpu'],
  timeout: 60000,
});

const page = await browser.newPage();
await page.setViewport({ width: 390, height: 844, deviceScaleFactor: 2 });
await page.goto('http://localhost:3000/scanner.html', { waitUntil: 'networkidle2', timeout: 30000 });

// Go to screen 2
await page.evaluate(() => goTo(2));
await new Promise(r => setTimeout(r, 800));

// Click the finance pain card (look for "פיננסים" label)
await page.evaluate(() => {
  const cards = document.querySelectorAll('.pain-card');
  for (const card of cards) {
    if (card.textContent.includes('פיננסים') || card.textContent.includes('finance')) {
      card.click();
      return;
    }
  }
  // Fallback: click first card
  if (cards[0]) cards[0].click();
});
await new Promise(r => setTimeout(r, 1000));

// Scroll to make finance tools visible
await page.evaluate(() => {
  const labels = document.querySelectorAll('.tools-cat-label');
  for (const label of labels) {
    if (label.textContent.includes('פיננסים')) {
      label.scrollIntoView({ behavior: 'instant', block: 'start' });
      return;
    }
  }
  // Fallback scroll
  window.scrollBy(0, 600);
});
await new Promise(r => setTimeout(r, 400));

const outPath = path.join(__dirname, 'temporary screenshots', 'finance-tools.png');
await page.screenshot({ path: outPath, fullPage: false });
await browser.close();
console.log('Saved:', outPath);
